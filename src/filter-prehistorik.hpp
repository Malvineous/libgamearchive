...
#ifndef _CAMOTO_FILTER_PREHISTORIK_HPP_
#define _CAMOTO_FILTER_PREHISTORIK_HPP_
...
class FilterType_Prehistorik: virtual public FilterType
{
	public:
		...
		virtual std::unique_ptr<stream::inout> apply(
			std::unique_ptr<stream::inout> target,
			stream::fn_notify_prefiltered_size resize) const;
		...
		virtual std::unique_ptr<stream::output> apply(
			std::unique_ptr<stream::output> target,
			stream::fn_notify_prefiltered_size resize) const;
};
...
#endif // _CAMOTO_FILTER_PREHISTORIK_HPP_
